...
void WeatherRenderer::Render(uint8_t* fb, int width, int height) {
    ...
    if (!has_data_) {
        const char* empty_text = i18n::Tr("暂无天气数据", "No weather data");
        ...
        if (location.empty()) location = i18n::Tr("杭州", "Hangzhou");
        ...
        char feels_buf[28];
        snprintf(feels_buf, sizeof(feels_buf), i18n::Tr("体感 %s°C", "Feels %s°C"),
        ...
        const char* aqi_label = i18n::Tr("空气质量", "Air Quality");
        ...
        std::string air = current_data_.air_quality.empty() ? i18n::Tr("优", "Good") : current_data_.air_quality;
        ...
        std::string desc_line = FitTextToWidth(current_data_.weather_text.empty() ? i18n::Tr("天气 --", "Weather --") : current_data_.weather_text,
                                               font_, 80);
        ...
        const char* labels[] = {i18n::Tr("湿度", "Humidity"), i18n::Tr("风向", "Wind Dir"),
                                 i18n::Tr("风力", "Wind"), i18n::Tr("紫外线", "UV")};
        char wind_scale_buf[24];
        if (current_data_.wind_scale.empty()) {
            std::snprintf(wind_scale_buf, sizeof(wind_scale_buf), "--");
        } else {
            std::snprintf(wind_scale_buf, sizeof(wind_scale_buf), i18n::Tr("%s级", "Lvl %s"),

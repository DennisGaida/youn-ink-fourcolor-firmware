...
void ChatRenderer::Render(uint8_t* fb, int width, int height) {
    ...
    if (is_listening_) {
        input_hint = i18n::Tr("正在录音并识别...", "Recording and recognizing...");
    } else if (is_streaming_) {
        input_hint = i18n::Tr("AI 正在回复", "AI is replying");
    } else if (!bottom_status_text_.empty()) {
        input_hint = bottom_status_text_.c_str();
    } else {
        input_hint = i18n::Tr("按住BOOT开始说话", "Hold BOOT to speak");
    }
    ...
    const char* action = is_streaming_ ? i18n::Tr("回复中", "Replying") : i18n::Tr("发送", "Send");
...
void ChatRenderer::DrawStreamingIndicator(uint8_t* fb, int width,
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "%s%s", i18n::Tr("思考中", "Thinking"), dots);
...
void ChatRenderer::SetListening(bool listening) {
    ...
    if (listening) {
        bottom_status_text_ = i18n::Tr("正在聆听...", "Listening...");
    } else {

...
void AudioService::PlaySound(const std::string_view& ogg) {
    ...
    int sample_rate = 16000; // default value
    ...
    while (true) {
        ...
        while (seg_idx < page_segments) {
            ...
            if (!seen_head) {
                // Parse the OpusHead packet
                if (pkt_len >= 19 && std::memcmp(pkt_ptr, "OpusHead", 8) == 0) {
                    seen_head = true;

                    // OpusHead structure: [0-7] "OpusHead", [8] version, [9] channel_count, [10-11] pre_skip
                    ...
                    if (pkt_len >= 12) {
                        ...
                        if (pkt_len >= 16) {
                            // Read the input sample rate (little-endian)

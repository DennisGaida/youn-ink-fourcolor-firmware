...
def build_assets(wakenet_model, text_font, emoji_collection, target_board, build_dir, final_dir):
    ...
    if text_font != "none":
        text_font_path = os.path.join("../../main/components/78__xiaozhi-fonts/cbin", f"{text_font}.bin")
        cmd.extend(["--text_font", text_font_path])
    
    if emoji_collection != "none":
        emoji_path = os.path.join("../../main/components/78__xiaozhi-fonts/build", emoji_collection)
    ...
    print(f"\nBuilding: {wakenet_model}-{text_font}-{emoji_collection}-{target_board}")
    ...
    try:
        ...
        if os.path.exists(src_path):
            ...
            print(f"✗ Error: generated assets.bin file not found")
            return False
...
def main():
    ...
    parser.add_argument('--mode',
                       ...
                       help='Select run mode: emoji_collections or emoji_target_boards (default: emoji_collections)')

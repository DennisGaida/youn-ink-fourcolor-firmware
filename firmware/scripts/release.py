...
def release(board_type: str, config_filename: str = "config.json", *, filter_name: Optional[str] = None) -> None:
    ...
    if filter_name:
        ...
        if not builds:
            print(f"[ERROR] Variant {filter_name} not found in {config_filename} for {board_type}", file=sys.stderr)
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", default="config.json", help="Specify the config file name, default config.json")
    parser.add_argument("--list-boards", action="store_true", help="List all supported boards and their variants")
    parser.add_argument("--json", action="store_true", help="Used with --list-boards, output in JSON format")
    parser.add_argument("--name", help="Specify the variant name; only compile the matching variant")
    ...
    for bt in sorted(target_board_types):
        ...
        if bt == board_type_input and not cfg_path.exists():
            print(f"Board {bt} does not define a {args.config} config file, skipping")
            sys.exit(0)
